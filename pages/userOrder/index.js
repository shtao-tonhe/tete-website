 ...
 * @LastEditTime: 2021-02-03 14:41:26
...
export default {
  ...
  methods: {
    async front_order_deliverGoods() {
      let res = await this.$api
        .front_order_deliverGoods({
      ...
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          if (res.code === 0) {
            this.visible = false;
            this.confirmLoading = false;
            this.$message.success(this.$t("components.tips.goodsDriveSuccess"));
            setTimeout(() => {
              this.front_order_business();
            }, 2000);
          } else {
    ...
    async front_order_business() {
      let res = await this.$api
        .front_order_business({
          perPage: this.perPage,
          page: this.page
        })
        .catch(err => {
